...
import 'package:noshmesh/core/updates/update_service.dart';
import 'package:noshmesh/l10n/l10n.dart';

/// Provider for the update service
final updateServiceProvider = Provider<UpdateService>((ref) {
  return BasicUpdateService(androidPackageName: AppConstants.packageName);
});
...
class UpdateDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopScope(
      canPop: !isCritical,
      child: AlertDialog(
        title: Text(isCritical ? context.tr('required_update') : context.tr('update_available')),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                isCritical
                    ? context.trParams('critical_update_message', {'version': updateInfo.latestVersion})
                    : context.trParams('new_version_available', {'version': updateInfo.latestVersion}),
                style: theme.textTheme.bodyLarge,
              ),
              if (updateInfo.releaseNotes != null) ...[
                const SizedBox(height: 16),
                Text(context.tr('whats_new'), style: theme.textTheme.titleSmall),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(isCritical ? context.tr('update_now') : context.tr('update')),
          ),
        ],
      ),
    );
